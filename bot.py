...
cur.execute('CREATE TABLE IF NOT EXISTS ex_channels(channel_id bigint)')
cur.execute('CREATE TABLE IF NOT EXISTS responses(response_regex text, response_title text, response_description text, response_num int)')
...
if cur.fetchone() is None:
    cur.execute(f'INSERT INTO settings(prefix, embedcolor, presence, yes) VALUES (%s, %s, %s, %s);', ('b?', '81C3FF', 'humans interact', 'yes'))  # set some defaults
db.commit()
