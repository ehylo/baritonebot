...
class DeletedLogger(commands.Cog):
    ...
    async def on_raw_message_delete(self, payload):
        ...
        if message.author.discriminator == '0000':
            return
        if message.author.id == bot_db.bot_id and message.channel.id != log_channel.id:
            return
        ...
        content = message.content if len(message.embeds) == 0 else message.embeds[0].description.split('**\n')[1]
