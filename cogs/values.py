...
class Prefix(commands.Cog):
    ...
    async def prefix(self, ctx, fixpre=None):
        if fixpre is None:
            ...
            const.cur.execute('UPDATE settings SET prefix=%s', (fixpre,))
            const.db.commit()
            await const.channel_embed(ctx, 'Prefix set', f'Set the prefix to {fixpre}, please restart the bot for the changes to take affect')
            logging.info(f'{ctx.author.id} set the prefix to {fixpre}')

    @prefix.command(aliases=['d'])
    @commands.check(const.admin_group)
    async def default(self, ctx):
        const.cur.execute("UPDATE settings SET prefix='b?'")
        const.db.commit()
        await const.channel_embed(ctx, 'Prefix set', 'Set the prefix to the default (b?), please restart the bot for the changes to take affect')
...
class EmbedColor(commands.Cog):
    ...
    async def embedcolor(self, ctx, color=None):
        if color is None:
            ...
            const.cur.execute('UPDATE settings SET embedcolor=%s', (color,))
            const.db.commit()
            await const.channel_embed(ctx, 'Embedcolor set', f'Set the embed color to {color}, please restart the bot for the changes to take affect')
            logging.info(f'{ctx.author.id} set the embedcolor to {color}')

    @embedcolor.command(aliases=['d'])
    @commands.check(const.admin_group)
    async def default(self, ctx):
        const.cur.execute("UPDATE settings SET embedcolor='81C3FF'")
        const.db.commit()
        await const.channel_embed(ctx, 'Embedcolor set', 'Set the embed color to the default (81C3FF), please restart the bot for the changes to take affect')
...
class Nick(commands.Cog):
    ...
    async def nick(self, ctx, *, name=None):
        ...
        if name is None:
            ...
            await const.channel_embed(ctx, 'Nick set', f'Set the bot\'s nickname in this server to `{name}`')
    ...
    async def default(self, ctx):
        ...
        await const.channel_embed(ctx, 'Nick set', 'Set the bot\'s nickname in this server to the default (Franky)')
    ...
    async def remove(self, ctx):
        ...
        await const.channel_embed(ctx, 'Nick removed', 'Removed the bot\'s nick in this server')
    ...
    async def status(self, ctx, ptype: int = None, *, presence=None):
        if (ptype is None) or (presence is None):
            ...
            if (ptype <= 0) or (ptype > 4):
                await const.error_embed(ctx, 'You need to give a number between 1 and 4 for the presence type, see `help status` for what each is')
            else:
                if ptype == 1:
                    ...
                    dtype = 'Competing in'
                const.cur.execute('UPDATE settings SET presence=%s', (presence,))
                const.cur.execute('UPDATE settings SET presencetype=%s', (dtype,))
                const.db.commit()
                await self.bot.change_presence(activity=discord.Activity(type=atype, name=presence))
                await const.channel_embed(ctx, 'Presence set', f'Set the presence to `{dtype} {presence}`.')
                logging.info(f'{ctx.author.id} set the status to {dtype} {presence}')

    @status.command(aliases=['d'])
    @commands.check(const.mod_group)
    async def default(self, ctx):
        const.cur.execute("UPDATE settings SET presence='humans interact'")
        const.cur.execute("UPDATE settings SET presencetype='Watching'")
        const.db.commit()
        await self.bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name='humans interact'))
        await const.channel_embed(ctx, 'Presence set', 'Set the presence to the default (`Watching humans interact`).')
