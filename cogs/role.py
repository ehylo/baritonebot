...
class Role(commands.Cog):
    ...
    async def ignore(self, ctx):
        if await role_add(self, ctx, main.ids(11), 'You already have the Ignore role', 'add'):
    ...
    async def unignore(self, ctx):
        if await role_add(self, ctx, main.ids(11), 'You don\'t have the Ignore role', 'remove'):
    ...
    async def releases(self, ctx):
        if await role_add(self, ctx, main.ids(13), 'You already have the Releases role', 'add'):
    ...
    async def unreleases(self, ctx):
        if await role_add(self, ctx, main.ids(13), 'You don\'t have the Releases role', 'remove'):
